"""Generate teaser video with animated 木 prompt icon."""
...
def lerp_color(c1, c2, t):
    """Interpolate between two RGB colors. t=0 gives c1, t=1 gives c2."""
    return tuple(int(a + (b - a) * t) for a, b in zip(c1, c2))


def breath_color(frame_i, speed=0.08):
    """Pulse between dim rose and full rose using a sine wave."""
    dim_rose = (120, 50, 60)
    t = (math.sin(frame_i * speed) + 1) / 2  # 0..1
...
def render_frame(lines, scroll_offset, cursor_parts=None, cursor_visible=True,
    ...
    """Render a frame. prompt_color_override changes 木 color on all visible prompts."""
...
def add_flash(n_frames=6):
    """Flash 木 white then fade back to rose — 'task complete' signal."""
    global scroll_y
    for i in range(n_frames):
...
def add_output_lines(output, delay=3, flash=True):
    """Add output lines one at a time with scroll. Flash 木 when done."""
    global scroll_y
    for line in output:
...
def add_spinner_progress(status_text, n_frames=36,
    ...
    """Spinner with 木 color-cycling + progress bar."""
    global scroll_y
    for i in range(n_frames):
        ...
        def draw_extra(draw, base_y, n_lines, p=progress, cc=cycle_color, st=status_text, pc=pct):
            # 木 spinner line (replaces braille)
...
def add_breathing_pause(n_frames):
    """Pause with 木 breathing (pulsing dim rose ↔ bright rose)."""
    global scroll_y
    for i in range(n_frames):
        tgt = target_scroll(current_lines)
        scroll_y = smooth_scroll(scroll_y, tgt)
...
# ─── Brand signoff: standalone 木 breathing ───
current_lines.append([])
...
mp4_path = "/Users/ankitpansari/Desktop/kinobi-landing/teaser_v3.mp4"
subprocess.run([
    "ffmpeg", "-y",
    "-framerate", str(FPS),
